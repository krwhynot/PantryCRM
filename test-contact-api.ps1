...
$existingContacts = Invoke-ApiTest -Method "GET" -Endpoint "/api/contacts" -Description "Existing Contacts List"# Test 5: Create a test contact (if we have an organization)
...
foreach ($test in $performanceTests) {
    ...
    try {
        ...
        if ($elapsed -lt $test.MaxTime) {
            Write-Host "✅ $($test.Description): ${elapsed}ms (Target: less than $($test.MaxTime)ms)" -ForegroundColor Green
        } else {
            Write-Host "⚠️  $($test.Description): ${elapsed}ms (Slow - Target: less than $($test.MaxTime)ms)" -ForegroundColor Yellow
        }
    }
    catch {
